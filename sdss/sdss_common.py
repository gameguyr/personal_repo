...
all_flag_value_array = [CANONICAL_CENTER,BRIGHT,EDGE,BLENDED,CHILD,PEAKCENTER,NODEBLEND,NOPROFILE,NOPETRO,MANYPETRO,NOPETRO_BIG,DEBLEND_TOO_MANY_PEAKS,COSMIC_RAY,MANYR50,MANYR90,BAD_RADIAL,INCOMPLETE_PROFILE,INTERP,SATURATED,NOTCHECKED,SUBTRACTED,NOSTOKES,BADSKY,PETROFAINT,TOO_LARGE,DEBLENDED_AS_PSF,DEBLEND_PRUNED,ELLIPFAINT,BINNED1,BINNED2,BINNED4,MOVED,DEBLENDED_AS_MOVING,NODEBLEND_MOVING,TOO_FEW_DETECTIONS,BAD_MOVING_FIT,STATIONARY,PEAKS_TOO_CLOSE,MEDIAN_CENTER,LOCAL_EDGE,BAD_COUNTS_ERROR,BAD_MOVING_FIT_CHILD,DEBLEND_UNASSIGNED_FLUX,SATUR_CENTER,INTERP_CENTER,DEBLENDED_AT_EDGE,DEBLEND_NOPEAK,PSF_FLUX_INTERP,TOO_FEW_GOOD_DETECTIONS,CENTER_OFF_AIMAGE,DEBLEND_DEGENERATE,BRIGHTEST_GALAXY_CHILD,CANONICAL_BAND,AMOMENT_FAINT,AMOMENT_SHIFT,AMOMENT_MAXITER,MAYBE_CR,MAYBE_EGHOST,NOTCHECKED_CENTER,OBJECT2_HAS_SATUR_DN,OBJECT2_DEBLEND_PEEPHOLE,GROWN_MERGED,HAS_CENTER]
all_flag_name_array = ['CANONICAL_CENTER','BRIGHT','EDGE','BLENDED','CHILD','PEAKCENTER','NODEBLEND','NOPROFILE','NOPETRO','MANYPETRO','NOPETRO_BIG','DEBLEND_TOO_MANY_PEAKS','COSMIC_RAY','MANYR50','MANYR90','BAD_RADIAL','INCOMPLETE_PROFILE','INTERP','SATURATED','NOTCHECKED','SUBTRACTED','NOSTOKES','BADSKY','PETROFAINT','TOO_LARGE','DEBLENDED_AS_PSF','DEBLEND_PRUNED','ELLIPFAINT','BINNED1','BINNED2','BINNED4','MOVED','DEBLENDED_AS_MOVING','NODEBLEND_MOVING','TOO_FEW_DETECTIONS','BAD_MOVING_FIT','STATIONARY','PEAKS_TOO_CLOSE','MEDIAN_CENTER','LOCAL_EDGE','BAD_COUNTS_ERROR','BAD_MOVING_FIT_CHILD','DEBLEND_UNASSIGNED_FLUX','SATUR_CENTER','INTERP_CENTER','DEBLENDED_AT_EDGE','DEBLEND_NOPEAK','PSF_FLUX_INTERP','TOO_FEW_GOOD_DETECTIONS','CENTER_OFF_AIMAGE','DEBLEND_DEGENERATE','BRIGHTEST_GALAXY_CHILD','CANONICAL_BAND','AMOMENT_FAINT','AMOMENT_SHIFT','AMOMENT_MAXITER','MAYBE_CR','MAYBE_EGHOST','NOTCHECKED_CENTER','OBJECT2_HAS_SATUR_DN','OBJECT2_DEBLEND_PEEPHOLE','GROWN_MERGED','HAS_CENTER']
all_flag_dict = {'name':all_flag_name_array, 'value':all_flag_value_array}

all_bad_flag_value_array = [SATURATED, SUBTRACTED, CHILD, PEAKCENTER, BLENDED, BINNED1, MOVED, NOTCHECKED, DEBLENDED_AT_EDGE, BRIGHT, NOPROFILE, DEBLEND_NOPEAK, STATIONARY, TOO_FEW_GOOD_DETECTIONS, INTERP, COSMIC_RAY]
all_bad_flag_name_array = ['SATURATED', 'SUBTRACTED', 'CHILD', 'PEAKCENTER', 'BLENDED', 'BINNED1', 'MOVED', 'NOTCHECKED', 'DEBLENDED_AT_EDGE', 'BRIGHT', 'NOPROFILE', 'DEBLEND_NOPEAK', 'STATIONARY', 'TOO_FEW_GOOD_DETECTIONS', 'INTERP', 'COSMIC_RAY']
all_bad_flag_dict = {'name':all_bad_flag_name_array, 'value':all_bad_flag_value_array}
...
def unpack_bit_mask(file_path):

    file_path_split_array =  file_path.split('.')
    output_file_path = file_path_split_array[0] + '_flagged' + '.' + file_path_split_array[1]

    pandas_data_frame = pandas.read_csv(file_path)



    for i in range(0, len(all_flag_dict['name'])):
        print 'trying to add this flag'
        print all_flag_dict['name'][i]
        array_of_flag_value = numpy.full(len(pandas_data_frame), int(all_flag_dict['value'][i]))
        flags_array = numpy.array(pandas_data_frame['flags'])
        bitwise_array = numpy.bitwise_and(flags_array, array_of_flag_value)
        boolean_array = bitwise_array != 0
        pandas_data_frame[all_flag_dict['name'][i]] = boolean_array

    pandas_data_frame.to_csv(output_file_path)
